...
```{r,  echo = TRUE}
summary(unlist(liste_distance$dist))
```
...
```{r, echo = TRUE}
bureaux_distincts <- resultats_bureaux %>%
  group_by(geom) %>%
  summarise(
...
#### Solution "parfaite" : agrégation des votes des doublons après arrondi des coordonnées à 10m

```{r, echo = TRUE}
bureaux_distincts <- resultats_bureaux %>%
  bind_cols(st_coordinates(.)) %>%
  st_drop_geometry() %>%
  mutate(X = round(X, digits = -1), Y = round(Y, digits = -1)) %>%
...
### Interpolation et autocorrélation spatiale
